...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/cart', middlewareIsAuthenticated, async (req: any, res) => {
    console.log("POST /api/cart payload:", req.body);
    try {
      const userId = parseInt(req.user.userData.id);
      if (!userId) {
        return res.status(401).json({ error: 'Unauthorized' });
      ...
      console.log("Parsed productId (as number):", Number(productId), " (original:", productId, ")");
      ...
      if (!product) {
        console.error("Add to cart failed: Product not found. Payload:", { productId, quantity, size, color }, " (productId as number:", Number(productId), ")");
      ...
      console.error("Error adding to cart (payload:", req.body, "):", error);
      res.status(500).json({ error: "Failed to add item to cart (see server log)" });
    }
  });
